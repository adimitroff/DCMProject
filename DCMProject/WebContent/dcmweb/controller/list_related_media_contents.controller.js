		...
		.define(
				...
				function(Controller, History, MessageToast) {
					...
					var PageController = Controller
							.extend(
									"net.cb.dcm.frontend.controller.list_related_media_contents",
									{	
										relPath:"",
										...
										onInit : function(evt) {
											...
											var loRoute = loRouter.getRoute("ListPlaylistRelatedMediaContents");
										...
										onAdd : function(evt) {
											if (!this._oDialog) {
												this._oDialog = sap.ui.xmlfragment("net.cb.dcm.frontend.fragment.media_content_popup_table", this);
											}
										...
										onDelete : function(event){
											...
											var lvDeletePath = this.relPath + "/$links/MediaContentDetails" + "(" + lvId + "L)"
										...
										handlePopupClose: function(oEvent) {
											...
											if (aContexts && aContexts.length) {
												...
												var oTable = this.byId("idMediaContentsTable");
												var oModel = this.getView().getModel();
												...
												if (sItems.length != 0) {
													for ( var i = sItems.length - 1; i >= 0; i--) {
														...
														if (lvId == lvTmpId){
															MessageToast.show("The media content choosen already existst in the table. No need to add it again!");
															return;
														}							
													}
												}
												var lvUrl = "/DCMProject/DCMService.svc" + 
													this.relPath + "/$links/MediaContentDetails";
												var lvNewMediaContentUri = "/MediaContents(" + lvId + "L)";
												var aData = jQuery.ajax({
													type : "POST",
													contentType : "application/json",
													data: '{ "uri": "' + lvNewMediaContentUri + '"}',
													url : lvUrl,
													dataType : "json",
													async: false,
										...
										updateTableBinding: function(){
											...
											var path = this.relPath + "/MediaContentDetails";
